...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    if (!self.calendar.floatingMode) {
        ...
        case UICollectionViewScrollDirectionHorizontal: {
            ...
            for (NSInteger column = startColumn; column <= endColumn; column++) {
                for (NSInteger row = 0; row < numberOfRows; row++) {
                    ...
                    UICollectionViewLayoutAttributes *rowSeparatorAttributes =
                        [self layoutAttributesForDecorationViewOfKind:kFSCalendarSeparatorInterRows atIndexPath:indexPath];
        ...
        NSInteger startRowIndex = ({
            CGFloat heightDelta1 = MIN(self.sectionBottoms[startSection] - CGRectGetMinY(rect) - self.sectionInsets.bottom,
                                       self.sectionRowCounts[startSection] * self.estimatedItemSize.height);
        ...
        NSInteger endRowIndex = ({
            CGFloat heightDelta2 =
                MAX(CGRectGetMaxY(rect) - self.sectionTops[endSection] - self.headerReferenceSize.height - self.sectionInsets.top, 0);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        if (!attributes) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForDecorationViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    if ([elementKind isEqualToString:kFSCalendarSeparatorInterRows] && (self.separators & FSCalendarSeparatorInterRows)) {
        ...
        if (!attributes) {
            ...
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:kFSCalendarSeparatorInterRows
                                                                                     withIndexPath:indexPath];
            CGFloat x, y;
            if (!self.calendar.floatingMode) {
                CGFloat rowOffset = [self calculateRowOffset:coordinate.row
                                                   totalRows:[self.calendar.calculator numberOfRowsInSection:indexPath.section]] +
                                    self.heights[coordinate.row];
                switch (self.scrollDirection) {
                case UICollectionViewScrollDirectionHorizontal: {
