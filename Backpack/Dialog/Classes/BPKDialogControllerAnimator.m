...
- (void)animateTransition:(nonnull id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    if (self.isPresenting) {
        ...
        if (dialogController) {
            [dialogController setScrimAlpha:0.0];
        }
        [controller.view layoutIfNeeded];

        [UIView animateKeyframesWithDuration:[[self class] duration] delay:0.0 options:0 animations:^{
            if (dialogController) {
                [dialogController setScrimAlpha:1.0];
                [dialogController addPresentingKeyFrameContentAnimationWithRelativeStartTime:0.4 relativeDuration:0.6];
            }
        ...
        [UIView animateKeyframesWithDuration:[[self class] duration] delay:0.0 options:0 animations:^{
            if (dialogController && dialogController.style == BPKDialogControllerStyleAlert) {
                ...
                [dialogController setScrimAlpha:0.0];
                [dialogController addDismissingKeyFrameContentAnimationWithRelativeStartTime:0.6 relativeDuration:0.4];
            }
        } completion:^(BOOL finished) {
            [transitionContext completeTransition:YES];
        }];
