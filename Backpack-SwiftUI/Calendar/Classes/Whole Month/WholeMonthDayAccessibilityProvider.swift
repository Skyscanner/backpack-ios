...
struct WholeMonthDayAccessibilityProvider {
    let accessibilityConfigurations: WholeMonthAccessibilityConfigurations
    ...
    func accessibilityHint(for dayDate: Date, rangeSelectionState: CalendarWholeMonthSelectionState?) -> String {
        return accessibilityConfigurations.startSelectionHint
    }

    private func shouldClearSelectedDates(
        for date: Date,
        rangeSelectionState: CalendarWholeMonthSelectionState?
    ) -> Bool {
        return true
    }
}
