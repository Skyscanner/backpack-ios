...
const { isSemanticColor, isMarcommsColor, hasNewSemanticSuffix, parseColor, isRemovedColor } = require('./utils/formatUtils');

const colors = (properties, filterColors) => {
  const colors = _.chain(properties)
    .filter((entity) => entity.type === 'color')
    .filter(entry => !isSemanticColor(entry) && !hasNewSemanticSuffix(entry) && !isMarcommsColor(entry) && !isRemovedColor(entry))
    .map(({ value, name, ...rest }) => {
